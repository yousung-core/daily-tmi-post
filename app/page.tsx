      ...
      <div className="space-y-12">
        ...
        <section className="fade-in py-4 border-b-2 border-ink-800">
          <p className="text-sm text-ink-600">
            📰 총 <strong>{allArticles.length}</strong>개의 특별한 이야기가 게시되었습니다
          </p>
        </section>
            ...
            <div className="flex flex-col">
              ...
              <article className="article-card bg-parchment-100 border-2 border-ink-800 p-6 flex-1 flex flex-col">
                ...
                <div className="flex items-center gap-4 text-sm text-ink-500 mt-auto">
                  <span>👤 {featuredArticles[0].protagonistName}</span>
                  <span>👁 {featuredArticles[0].viewCount}</span>
                  <span>
                    {new Date(featuredArticles[0].createdAt).toLocaleDateString("ko-KR")}
                  </span>
                </div>
              </article>
            </div>
                ...
                {featuredArticles.slice(1, 5).map((article, index) => (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-lg">{submissionCategoryIcons[article.category]}</span>
                    ...
                    <div className="text-xs text-ink-500">
                      👤 {article.protagonistName} · 👁 {article.viewCount} · {new Date(article.createdAt).toLocaleDateString("ko-KR")}
                    </div>
                  </article>
                ))}
              </div>
